...
label_font = pygame.font.Font('PlaywriteITModerna-VariableFont_wght.ttf', 28)
medium_font = pygame.font.Font('PlaywriteITModerna-VariableFont_wght.ttf', 20)
...
hi_hat = mixer.Sound('sounds/hi hat (16).wav')
snare = mixer.Sound('snare.WAV')
kick = mixer.Sound('sounds/kick (16).wav')
crash = mixer.Sound('sounds/cymbal (3).wav')
clap = mixer.Sound('sounds/clap (15).wav')
tom = mixer.Sound('sounds/tom (10).WAV')
ride = mixer.Sound('ride.mp3')
...
def draw_grid(clicks, beat, actives):
    ...
    screen.blit(kick_text, (30, 210))
    
    crash_text = label_font.render('Crash', True, colors[actives[3]])
    screen.blit(crash_text, (30, 310))
    
    clap_text = label_font.render('Clap', True, colors[actives[4]])
    screen.blit(clap_text, (30, 410))
    
    tom_text = label_font.render('Floor Tom', True, colors[actives[5]])
    screen.blit(tom_text, (30, 510))

    ride_text = label_font.render('Ride', True, colors[actives[6]])
    screen.blit(ride_text, (30, 610))
...
def draw_load_menu(index):
    ...
    for beat in range(len(saved_beats)):
        ...
        if 0 <= index < len(saved_beats) and beat == index:
            beats_index_end = saved_beats[beat].find(', bpm:')
...
while run:
    ...
    # clear board button
    clear = pygame.draw.rect(screen, gray, [1150, HEIGHT - 150, 200, 48], 0, 5)
    play_text = label_font.render('Clear Board', True, white)
    screen.blit(play_text, (1160, HEIGHT - 145))

    # reset button
    reset = pygame.draw.rect(screen, gray, [1150, HEIGHT - 98, 200, 48], 0, 5)
    play_text = label_font.render('Reset Board', True, white)
    screen.blit(play_text, (1160, HEIGHT - 95))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            for i in range (len(boxes)):
                if boxes[i][0].collidepoint(event.pos):
                    ...
                    clicked[coordinates[1]][coordinates[0]] *= -1
